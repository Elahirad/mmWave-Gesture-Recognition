            ...
            for dir in os.listdir(os.path.join(directory, item)):
                if dir not in ["other", "run"]:
                    ...
                    if len(files) < c:
                        c = len(files)
                    user["dirs"][dir] = list(
                        map(lambda f: os.path.join(directory, item, dir, f), files)
                    )
            user["min_samples"] = c
...
def pick_main_motions(directory, samples_from_each_user, output_folder_name):
    ...
    for user in map_users_to_files(directory):
        samples_to_pick = (
            user["min_samples"]
            if user["min_samples"] < samples_from_each_user
            else samples_from_each_user
        )
        ...
        if os.path.isdir(os.path.join(directory, item)):
            for dir in os.listdir(os.path.join(directory, item)):
                if dir in ["other", "run"]:
                    ...
                    files = list(
                        map(
                            lambda f: os.path.join(directory, item, dir, f),
                            os.listdir(os.path.join(directory, item, dir)),
                        )
                    )
                    for file in files:
                        source = file
                        dest = os.path.join(
                            os.getcwd(), "Out", "unex", f"sample_{k}.npz"
                        )
                        shutil.copy2(source, dest)
                        k += 1
                        print(f"Copied {source} -> {dest}")
