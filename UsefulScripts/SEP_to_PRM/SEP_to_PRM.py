...
def calculate_prm_status(segment_frame):
    ...
    segment_frame['R'] = calculate_range(segment_frame['X'], segment_frame['Y'], segment_frame['Z'])
    ...
    NCP = len(centripetal)
    RCP = np.average(centripetal['R'], weights=centripetal['Intensity']) if NCP > 0 else 0
    VCP = np.average(centripetal['Doppler'], weights=centripetal['Intensity']) if NCP > 0 else 0

    # Centrifugal movement (V <= 0)
    centrifugal = segment_frame[segment_frame['Doppler'] <= 0]
    NCF = len(centrifugal)
    RCF = np.average(centrifugal['R'], weights=centrifugal['Intensity']) if NCF > 0 else 0
    VCF = np.average(centrifugal['Doppler'], weights=centrifugal['Intensity']) if NCF > 0 else 0
...
def process_and_save_prm_matrices(file_path, output_dir):
    ...
    for segment in segments:
        ...
        while len(prm_matrix) > segment_len:
            ...
            np.savez_compressed(f'{output_dir}/sample_{sample_count}.npz', prm_matrix=matrix)
        ...
        if len(prm_matrix) < segment_len:
            ...
            prm_matrix = [[0]*10]*zeros_before + prm_matrix + [[0]*10]*zeros_after

        sample_count += 1
        np.savez_compressed(f'{output_dir}/sample_{sample_count}.npz', prm_matrix=prm_matrix)
